...
public class Spiral : MonoBehaviour {
	...
	[Range(.001f, 10f)] [SerializeField] float width = 1.5f;
	[Range(.001f, 10f)] [SerializeField] float height = .5f;
	[Range(.01f, 100)] [SerializeField] float length = 9.06f;
	[Range(4, 720)] [SerializeField] int sides = 45;
	[Range(.00f, 100)] [SerializeField] float offset = 2f;
	...
	void CreateScaffold()
	{
		...
		for (var i = 0; i < dist; i++)
		{
			...
			float tRadius = radius * (i * 0.01f);
			// Radius surface helpers
			vertexSurfaceRadius = pos * tRadius;
			vertexSurfaceRadius += upOffset;

			// Inner top vertex
			vertexInnerTop = pos * (tRadius - halfWidth);
			vertexInnerTop += upOffset;
			
			// Outer top vertex
			vertexOuterTop = pos * (tRadius + halfWidth);
			vertexOuterTop += upOffset;

			// Inner bottom vertex
			vertexInnerBottom = pos * (tRadius - halfWidth);
			vertexInnerBottom += downOffset;

			// Outer bottom vertex
			vertexOuterBottom = pos * (tRadius + halfWidth);
